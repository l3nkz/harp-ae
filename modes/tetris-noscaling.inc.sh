...
function check_tetris-noscaling() {
    local failure=0
...
function __warmup_prog() {
    ...
    while [ $repeat -eq 1 ]; do
        ...
        # Disable TETRiS scaling support by providing an incorrect path to the TETRIS_LIBS
        old_tetris_libs=$TETRIS_LIBS
        export TETRIS_LIBS=/dev/null
        ...
        # Restore TETRIS_LIBS
        export TETRIS_LIBS=$old_tetris_libs
...
function warmup_tetris-noscaling() {
    local log_base_dir="$2"
...
function run_tetris-noscaling() {
    ...
    if [ $all_finished -eq 1 ]; then
        echo "tetris-noscaling;$total_t;$total_e" >> $result_file
        echo 1
    else
        echo 0
    fi
